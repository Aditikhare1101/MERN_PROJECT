...
export default function Admin() {
  ...
  useEffect(()=>{
    axios.get("http://localhost:5001/admin/read").then((response)=>{
...
<br/>
      ...
      {adminList.map((val, key)=>{
        return <div key = {key} className = "admin">
          ...
          <p></p>
          <h5  className= "adminlist"> Password: {val.password}</h5>
          <p></p>
          ...
          <button type="button" className="btn btn-success" onClick= {()=>updateAdmin(val._id)}>Update</button><br/> <p></p>
          <button type="button" className="btn btn-danger"onClick={()=> deleteAdmin(val._id)} >Delete</button>
          <p></p>

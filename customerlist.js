...
const [newfirstname,setnewfirstname]= useState('');
  ...
  useEffect(()=>{
    axios.get("http://localhost:5001/customer/read").then((response)=>{
...
<br/>
      ...
      {customerList.map((val, key)=>{
        return <div key = {key} className = "book">
          ...
          <p> </p>
          <h5  className= "customerlist">Password: {val.password}</h5>
          <p> </p>
          ...
          <p> </p>
          <button type="button" className="btn btn-success" onClick= {()=>updateCustomer(val._id)}>Update</button><br/>
          <p> </p>
          
          <button  type="button" className="btn btn-danger" onClick={()=> deleteCustomer(val._id)} >Delete</button>
          <p> </p>
